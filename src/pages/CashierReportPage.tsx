  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-16 animate-in fade-in duration-500">
      ...
      <div className="px-2 mt-1.5">
         <p className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-1 px-0.5">
           {activeCategory} ({filteredTransactions.length})
         </p>
         <div className="grid grid-cols-3 gap-1">
            {filteredTransactions.map((tx, index) => (
              <button 
                ...
                className="bg-white rounded-lg border border-slate-100 p-1.5 flex flex-col shadow-sm active:scale-[0.97] transition-all overflow-hidden"
              >
                <div className="flex items-center justify-between w-full mb-0.5">
                  <span className="text-[8px] font-black text-[#1AB1A5] leading-none">{tx.room !== "NONE" ? tx.room : "LẺ"}</span>
                  <div className={cn(
                        ...
                        {tx.method === "CA" ? <Banknote className="w-2 h-2 text-emerald-500" /> : 
                         tx.method === "BT" ? <Landmark className="w-2 h-2 text-blue-500" /> :
                         tx.method === "AC" ? <Building2 className="w-2 h-2 text-indigo-500" /> :
                         <CreditCard className="w-2 h-2 text-amber-500" />}
                   </div>
                </div>
                <div className="text-[8px] font-black text-slate-700 leading-none">{tx.amount}đ</div>
                <div className="text-[6px] font-bold text-slate-300 leading-none mt-0.5 truncate w-full">{tx.id} · {tx.time}</div>
              </button>
            ))}
         </div>
      </div>
